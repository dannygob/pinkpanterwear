...
import com.example.pinkpanterwear.UserProductDetailsActivity
...
class UserCategoryProductsFragment : Fragment() {
    ...
    private fun observeViewModel() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            viewModel.isLoading.collectLatest { isLoading ->
                ...
                recyclerView.isVisible = !isLoading && categoryName != null && viewModel.error.value == null
            }
        }
        viewLifecycleOwner.lifecycleScope.launch {
    ...
    private fun navigateToProductDetails(product: Product) {
        val intent = Intent(activity, UserProductDetailsActivity::class.java).apply {
             putExtra("PRODUCT_ID", product.id.toString()) // API getProductDetails expects Int, Activity might still expect String
        }
