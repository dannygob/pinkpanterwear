...
import android.widget.Button // Added for checkout button
...
class UserCartFragment : Fragment() {
        ...
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.cartItems.collectLatest { items ->
                    ...
                    updateUIStateBasedOnCart(items, viewModel.isLoading.value, viewModel.error.value)
                }
            }
        }
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.isLoading.collectLatest { isLoading ->
                     updateUIStateBasedOnCart(viewModel.cartItems.value, isLoading, viewModel.error.value)
                }
            }
        }
        viewLifecycleOwner.lifecycleScope.launch {
            repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.error.collectLatest { error ->
                    updateUIStateBasedOnCart(viewModel.cartItems.value, viewModel.isLoading.value, error)
    ...
    private fun updateUIStateBasedOnCart(items: List<CartItem>, isLoading: Boolean, error: String?) {
